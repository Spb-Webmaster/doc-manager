...
<style>
  ...
  /* Left: list */
  .list-col { width: 380px; flex-shrink: 0; border-right: 1px solid var(--border); display: flex; flex-direction: column; overflow: hidden; }
  .list-toolbar { padding: 14px 16px; border-bottom: 1px solid var(--border); display: flex; flex-direction: column; gap: 10px; background: var(--surface); }
  .search-wrap { position: relative; }
  .search-ico { position: absolute; left: 11px; top: 50%; transform: translateY(-50%); color: var(--text-s); pointer-events: none; }
  .search-input { width: 100%; padding: 9px 12px 9px 36px; border: 1.5px solid var(--border); border-radius: var(--rad-sm); font-family: inherit; font-size: 14px; color: var(--text-h); background: var(--bg); outline: none; transition: border-color .15s, box-shadow .15s; }
  .search-input:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(37,80,226,.10); background: var(--surface); }
  .search-input::placeholder { color: var(--text-s); }
  .list-count { font-size: 12px; color: var(--text-s); padding: 0 2px; }

  /* CP list */
  .cp-list { flex: 1; overflow-y: auto; }
  .cp-item { display: flex; align-items: center; gap: 12px; padding: 14px 16px; border-bottom: 1px solid var(--border); cursor: pointer; transition: background .12s; background: var(--surface); }
  .cp-item:last-child { border-bottom: none; }
  .cp-item:hover { background: var(--bg); }
  .cp-item.active { background: var(--accent-lt); border-right: 3px solid var(--accent); }
  .cp-item.active .cp-name { color: var(--accent); }
  .cp-ava { width: 40px; height: 40px; border-radius: 11px; flex-shrink: 0; display: flex; align-items: center; justify-content: center; font-size: 14px; font-weight: 700; }
  .cp-info { flex: 1; min-width: 0; }
  .cp-name { font-size: 13.5px; font-weight: 600; color: var(--text-h); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-bottom: 2px; }
  .cp-meta { font-size: 12px; color: var(--text-s); display: flex; align-items: center; gap: 8px; }
  .cp-docs-count { display: flex; align-items: center; gap: 3px; font-size: 12px; color: var(--text-s); flex-shrink: 0; }

  /* Add btn inline */
  .add-cp-item { display: flex; align-items: center; gap: 12px; padding: 14px 16px; cursor: pointer; transition: background .12s; background: var(--surface); border-top: 1px dashed var(--border); color: var(--accent); font-size: 13.5px; font-weight: 600; }
  .add-cp-item:hover { background: var(--accent-lt); }
  .add-cp-ico { width: 40px; height: 40px; border-radius: 11px; background: var(--accent-lt); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }

  /* Right: detail */
  .detail-col { flex: 1; overflow-y: auto; background: var(--bg); display: flex; flex-direction: column; }

  /* Empty detail */
  .detail-empty { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 12px; color: var(--text-s); text-align: center; padding: 40px; }
  .detail-empty svg { opacity: .25; }
  .detail-empty-title { font-size: 15px; font-weight: 600; color: var(--text-s); }
  .detail-empty-sub { font-size: 13px; max-width: 280px; line-height: 1.6; }

  /* Detail header */
  .detail-head { background: var(--surface); border-bottom: 1px solid var(--border); padding: 20px 24px; display: flex; align-items: center; gap: 14px; flex-shrink: 0; flex-wrap: wrap; }
  .detail-ava { width: 52px; height: 52px; border-radius: 14px; display: flex; align-items: center; justify-content: center; font-size: 18px; font-weight: 700; flex-shrink: 0; }
  .detail-head-info { flex: 1; min-width: 0; }
  .detail-name { font-size: 18px; font-weight: 700; color: var(--text-h); letter-spacing: -.3px; margin-bottom: 3px; }
  .detail-inn  { font-size: 13px; color: var(--text-s); }
  ...
  /* Info card */
  .info-card { background: var(--surface); border: 1px solid var(--border); border-radius: var(--rad); overflow: hidden; }
  .info-card-head { padding: 13px 18px; border-bottom: 1px solid var(--border); font-size: 13px; font-weight: 700; color: var(--text-h); }
  .info-card-body { padding: 4px 18px 14px; }
  .info-row { display: flex; align-items: flex-start; gap: 12px; padding: 10px 0; border-bottom: 1px solid var(--border); }
  .info-row:last-child { border-bottom: none; }
  .info-key { width: 84px; flex-shrink: 0; font-size: 11.5px; font-weight: 700; text-transform: uppercase; letter-spacing: .5px; color: var(--text-s); padding-top: 2px; }
  .info-val { font-size: 13.5px; font-weight: 500; color: var(--text-h); flex: 1; word-break: break-word; }

  /* Stats */
  .cp-stats-row { display: grid; grid-template-columns: repeat(3,1fr); gap: 10px; }
  .cp-stat { background: var(--surface); border: 1px solid var(--border); border-radius: var(--rad); padding: 14px 16px; }
  .cp-stat-val { font-size: 22px; font-weight: 700; color: var(--text-h); letter-spacing: -.8px; line-height: 1; margin-bottom: 4px; }
  .cp-stat-lbl { font-size: 12px; color: var(--text-s); }

  /* Docs table */
  .doc-mini-table { background: var(--surface); border: 1px solid var(--border); border-radius: var(--rad); overflow: hidden; }
  .doc-mini-head { display: grid; grid-template-columns: 1fr 90px 110px 90px; padding: 9px 16px; background: var(--bg); border-bottom: 1px solid var(--border); font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: .6px; color: var(--text-s); gap: 8px; }
  .doc-mini-row { display: grid; grid-template-columns: 1fr 90px 110px 90px; padding: 12px 16px; border-bottom: 1px solid var(--border); font-size: 13px; gap: 8px; align-items: center; }
  .doc-mini-row:last-child { border-bottom: none; }
  .dmr-num  { font-weight: 700; color: var(--text-h); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .dmr-date { color: var(--text-s); }
  .dmr-sum  { font-weight: 600; color: var(--text-h); text-align: right; }
  /* Invoice rows: override grid to replace "Статус" on "Договор" */
  .doc-inv-head, .doc-inv-row { grid-template-columns: 1.2fr 80px 100px 1.4fr auto; }
  .dmr-del { display: flex; align-items: center; justify-content: flex-end; gap: 6px; }
  .btn-pdf { color: var(--accent); border-color: rgba(37,80,226,.25); }
  .btn-pdf:hover { background: var(--accent-lt); border-color: var(--accent); }
  .dmr-link  { color: var(--accent); text-decoration: none; font-weight: 700; }
  .dmr-link:hover { text-decoration: underline; }
  .dmr-basis { font-size: 12.5px; color: var(--text-s); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .doc-empty { padding: 24px 16px; text-align: center; font-size: 13px; color: var(--text-s); }
  .badge { display: inline-flex; align-items: center; padding: 3px 9px; border-radius: 6px; font-size: 11.5px; font-weight: 600; white-space: nowrap; }
  .badge-inv { background: var(--accent-lt); color: var(--accent); }
  .badge-act { background: var(--green-lt);  color: var(--green); }
  /* Doc tabs */
  .doc-tabs { display: flex; border-bottom: 1px solid var(--border); }
  .doc-tab { flex: 1; padding: 10px 16px; border: none; border-bottom: 2px solid transparent; margin-bottom: -1px; background: none; font-family: inherit; font-size: 13px; font-weight: 600; color: var(--text-s); cursor: pointer; transition: color .15s, border-color .15s; }
  .doc-tab.active { color: var(--accent); border-bottom-color: var(--accent); }
  .doc-tab:hover:not(.active) { color: var(--text-h); }
  .doc-tab-cnt { font-size: 11px; font-weight: 700; background: var(--bg); color: var(--text-s); border: 1px solid var(--border); border-radius: 10px; padding: 1px 6px; margin-left: 4px; vertical-align: middle; }
  .doc-tab.active .doc-tab-cnt { background: var(--accent-lt); color: var(--accent); border-color: transparent; }
  /* Status badges */
  .badge-draft     { background: var(--bg); color: var(--text-s); border: 1px solid var(--border); }
  .badge-sent      { background: #FFF3E0; color: #D4700A; }
  .badge-paid      { background: var(--green-lt); color: var(--green); }
  .badge-signed    { background: var(--green-lt); color: var(--green); }
  .badge-cancelled { background: var(--red-lt); color: var(--red); }

  .autofill-result { background: var(--green-lt); border: 1px solid rgba(21,155,106,.2); border-radius: var(--rad-sm); padding: 12px 14px; display: none; }
  .autofill-result.show { display: block; }
  .af-name { font-size: 14px; font-weight: 700; color: var(--text-h); margin-bottom: 5px; display: flex; align-items: center; gap: 6px; }
  .af-row  { font-size: 12.5px; color: var(--text-b); }
  .af-row span { color: var(--text-s); }

  .inn-status-wrap { position: relative; }
  .inn-status { position: absolute; right: 12px; top: 50%; transform: translateY(-50%); width: 20px; height: 20px; display: flex; align-items: center; justify-content: center; }
  .spinner { width: 18px; height: 18px; border: 2px solid var(--border); border-top-color: var(--accent); border-radius: 50%; animation: spin .7s linear infinite; display: none; }
  @keyframes spin { to { transform: rotate(360deg); } }

  /* Contracts */
  .contracts-head { padding: 13px 18px; border-bottom: 1px solid var(--border); font-size: 13px; font-weight: 700; color: var(--text-h); display: flex; align-items: center; justify-content: space-between; }
  .contract-row { display: flex; align-items: center; gap: 12px; padding: 11px 18px; border-bottom: 1px solid var(--border); }
  .contract-row:last-child { border-bottom: none; }
  .contract-row-main { flex: 1; min-width: 0; }
  .contract-name { font-size: 13.5px; font-weight: 600; color: var(--text-h); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .contract-meta { font-size: 12px; color: var(--text-s); margin-top: 2px; }
  .contract-row-actions { display: flex; gap: 5px; flex-shrink: 0; }
  .contract-row-actions .btn, .dmr-del .btn { padding: 4px 10px; font-size: 12px; }
  .contract-drag-handle { cursor: grab; color: var(--text-s); opacity: .45; flex-shrink: 0; display: flex; align-items: center; padding: 0 2px; }
  .contract-drag-handle:hover { opacity: .7; }
  .contract-row.dragging { opacity: .35; }
  .contract-row.drag-over { border-color: var(--accent); box-shadow: 0 0 0 2px rgba(37,80,226,.15); }
